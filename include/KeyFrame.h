...
public:
    ...
    float inline GetScaleFactor(int nLevel=1) const{
        return m_scaleFactorVector[nLevel];}
    std::vector<float> inline GetScaleFactors() const{
        return m_scaleFactorVector;}
...
public:
    ...
    int getMnFrameId(){
    	return this->mnFrameId;
    }
    void setFrameId(int nbFrameId){
    	this->mnFrameId=nbFrameId;
    }
...
protected:
    ...
    std::map<KeyFrame*,int> m_keyframeWeightConnectedMap;
    std::vector<KeyFrame*> m_OderedKeyFrameVector;
    std::vector<int> m_orderedWeightVector;

    // Spanning Tree and Loop Edges
    bool m_firstConnection;
    KeyFrame* m_ParentKeyframe;
    ...
    //The value of the elements in a set cannot be modified once in the container (the elements are always const), but they can be inserted or
    //removed from the container.
    std::set<KeyFrame*> m_childrensSet;
    ...
    // Scale
    int mnScaleLevels;
    std::vector<float>  m_scaleFactorVector;
