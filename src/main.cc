...
int main(int argc, char **argv)
{
	...
	//Data/ORBvoc.yml Data/Settings_KITTI.yaml;
	...
	// Load Settings and Check
	string vocPath="Data/ORBvoc.yml";
	string settingPath="Data/Settings_KITTI.yaml";
	//Data/ORBvoc.yml Data/Settings_KITTI.yaml;

	string strSettingsFile = ros::package::getPath("ORB_SLAM")+"/"+settingPath;
	...
	string strVocFile = ros::package::getPath("ORB_SLAM")+"/"+vocPath;
	...
	for(size_t i=0; i<vpKFs.size(); i++)
	{
		...
		cv::Mat t1=t.clone();
		double scale = pKF->getAbsoluteScale(pKF->mnFrameId);
		if ((scale>0.1)) {
			std::cout << "Scale " << scale << std::endl;
			t1 = t1 + scale*t1;

		}
		f << setprecision(6) << pKF->mTimeStamp << setprecision(7) << " " << t1.at<float>(0) << " " << t1.at<float>(1) << " " << t1.at<float>(2)
