...
PnPsolver::PnPsolver(const Frame &F, const vector<MapPoint*> &vpMapPointMatches):
    ...
    mnIterations(0), mnBestInliers(0), numberOfCorrespondance(0)
{
    mvpMapPointMatches = vpMapPointMatches;
    m_vector2DPoint.reserve(F.m_MapPoints.size());
    mvSigma2.reserve(F.m_MapPoints.size());
    m_vector3DPoint.reserve(F.m_MapPoints.size());
    mvKeyPointIndicesInFrame.reserve(F.m_MapPoints.size());
    mvAllIndices.reserve(F.m_MapPoints.size());
    ...
    for(size_t i=0, iend=vpMapPointMatches.size(); i<iend; i++)
    {
        ...
        if(pMP)
        {
            if(!pMP->isBad())
            {
                const cv::KeyPoint &kp = F.m_vectorKeypointsUndistorted[i];

                m_vector2DPoint.push_back(kp.pt);
                mvSigma2.push_back(F.mvLevelSigma2[kp.octave]);

                cv::Mat Pos = pMP->GetWorldPos();
                m_vector3DPoint.push_back(cv::Point3f(Pos.at<float>(0),Pos.at<float>(1), Pos.at<float>(2)));

                mvKeyPointIndicesInFrame.push_back(i);
...
void PnPsolver::SetRansacParameters(double probability, int minInliers, int maxIterations, int minSet, float epsilon, float th2)
{
    ...
    numberOfCorrespondance = m_vector2DPoint.size(); // number of correspondences

    mvbInliersi.resize(numberOfCorrespondance);

    // Adjust Parameters according to number of correspondences
    int nMinInliers = numberOfCorrespondance*mRansacEpsilon;
    ...
    if(mRansacEpsilon<(float)mRansacMinInliers/numberOfCorrespondance)
        mRansacEpsilon=(float)mRansacMinInliers/numberOfCorrespondance;

    // Set RANSAC iterations according to probability, epsilon, and max iterations
    int nIterations;

    if(mRansacMinInliers==numberOfCorrespondance)
        nIterations=1;
    else
...
cv::Mat PnPsolver::iterate(int nIterations, bool &bNoMore, vector<bool> &vbInliers, int &nInliers)
{
    ...
    if(numberOfCorrespondance<mRansacMinInliers)
    {
        bNoMore = true;
        return cv::Mat();
    }
    ...
    while(mnIterations<mRansacMaxIts || nCurrentIterations<nIterations)
    {
        ...
        for(short i = 0; i < mRansacMinSet; ++i)
        {
            ...
            add_correspondence(m_vector3DPoint[idx].x,m_vector3DPoint[idx].y,m_vector3DPoint[idx].z,m_vector2DPoint[idx].x,m_vector2DPoint[idx].y);
        ...
        if(mnInliersi>=mRansacMinInliers)
        {
            ...
            if(Refine())
            {
                ...
                for(int i=0; i<numberOfCorrespondance; i++)
                {
                    if(mvbRefinedInliers[i])
                        vbInliers[mvKeyPointIndicesInFrame[i]] = true;
                }
...
void PnPsolver::CheckInliers()
{
    ...
    for(int i=0; i<numberOfCorrespondance; i++)
    {
        cv::Point3f P3Dw = m_vector3DPoint[i];
        cv::Point2f P2D = m_vector2DPoint[i];
