...
{    
    ...
    // SET FRAME VERTEX
    g2o::VertexSE3Expmap * vSE3 = new g2o::VertexSE3Expmap();
    vSE3->setEstimate(Converter::toSE3Quat(pFrame->m_cameraPose));
    vSE3->setId(0);
    vSE3->setFixed(false);
    optimizer.addVertex(vSE3);
    ...
    const int N = pFrame->m_MapPoints.size();
    vpEdges.reserve(N);
    vVertices.reserve(N);
    ...
    for(int i=0; i<N; i++)
    {
        MapPoint* pMP = pFrame->m_MapPoints[i];
        if(pMP)
        {
            g2o::VertexSBAPointXYZ* vPoint = new g2o::VertexSBAPointXYZ();
            vPoint->setEstimate(Converter::toVector3d(pMP->GetWorldPos()));
            ...
            //SET EDGE
            Eigen::Matrix<double,2,1> obs;
            cv::KeyPoint kpUn = pFrame->m_vectorKeypointsUndistorted[i];
            obs << kpUn.pt.x, kpUn.pt.y;
    ...
    pose.copyTo(pFrame->m_cameraPose);
